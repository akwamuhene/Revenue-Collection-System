...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <title>Birim Central Municipal Assembly - Revenue Annual</title>
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                             ANNUAL FEES, FINES & LICENCES GRAPH AS AT <?php echo date("M");?> <?php echo $_SESSION['year'];?>
                        </div>
                        <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-1">
                                    ...
                                    <tbody>
                                        <?php 
                                        $sql ="SELECT tolltype, moment, sum(amtc) AS tolltypetotal FROM tolltable LEFT JOIN tolltype ON tolltype.tolltypeid=tolltable.tolltypeid WHERE year(`moment`)=".$_SESSION['year']." GROUP BY tolltype";
                                        $result = mysqli_query($conn, $sql);
                                     while ($row = mysqli_fetch_array($result)) { 
                                     $tolltotal+=$row['tolltypetotal'];
                             ?>
                                <tr>
                                    <td><?php echo $row['tolltype']; ?></td>
                                    <td><?php echo number_format((float)$row['tolltypetotal'], 2, '.', ''); ?></td>
                                </tr>
                                <?php
                             }
                                ?>
                                ...
                                <tr>
                                    <td><b>TOTAL (GH₵)</b></td>
                                    <td><b><?php echo number_format((float)$tolltotal, 2, '.', ''); ?></b></td>
                                </tr>
                                </tfoot>
                                </tbody>
                                </table>
                            </div><hr style="height:10px;background-color:#000;">
                        <?php $sql ="SELECT tolltype, moment, sum((amtc * 100) / (select sum(amtc) from tolltable)) AS tolltypetotal FROM tolltable LEFT JOIN tolltype ON tolltype.tolltypeid=tolltable.tolltypeid WHERE year(`moment`)=".$_SESSION['year']." GROUP BY tolltype";
                             $result = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($result)) { 
                     
                                $tolltype[]  = $row['tolltype'];
                                $amount[] = number_format((float)$row['tolltypetotal'], 2, '.', '');
                             }
                                ?>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                             ANNUAL RATES TABLE AS AT <?php echo date("M");?> <?php echo $_SESSION['year'];?>
                        </div>
                        <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-2">
                                    ...
                                    <tbody>
                                <?php 
                                    $sql ="SELECT rates.rate, moment, sum(paid) AS ratypetotal, sum(amt) AS amtt FROM ratetable LEFT JOIN rates ON rates.rate=ratetable.rate WHERE year(`moment`)=".$_SESSION['year']." GROUP BY rates.rate";
                                        $result = mysqli_query($conn, $sql);
                                     while ($row = mysqli_fetch_array($result)) { 
                                    $baltt = $row['ratypetotal']-$row['amtt'];
                                     $paidtotal+=$row['ratypetotal'];
                                     $targettotal+=$row['amtt'];
                                     $balancetotal+=$baltt;
                                ?>
                                <tr>
                                    <td><?php echo $row['rate']; ?></td>
                                    <td><?php echo number_format((float)$row['ratypetotal'], 2, '.', ''); ?></td>
                                    <td><?php echo number_format((float)$row['amtt'], 2, '.', ''); ?></td>
                                    <td><?php echo number_format((float)$baltt, 2, '.', ''); ?></td>
                                </tr>
                                <?php
                             }
                                ?>
                                ...
                                <tr>
                                    <td><b>TOTAL (GH₵)</b></td>
                                    <td><b><?php echo number_format((float)$paidtotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$targettotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$balancetotal, 2, '.', ''); ?></b></td>
                                </tr>
                                </tfoot>
                                </tbody>
                                </table>
                        ...
                        <?php 
                       /* $sql ="SELECT rates.rate, moment, sum((paid * 100)/(select amt from ratetable where rates.rate=ratetable.rate)) AS ratypetotal FROM ratetable LEFT JOIN rates ON rates.rate=ratetable.rate WHERE year(`moment`)=".$_SESSION['year']." GROUP BY rates.rate";
                             $res = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($res)) { 
                     
                                $rate[]  = $row['rate'];
                                $ramount[] = number_format((float) $row['ratypetotal'], 2, '.', ''); 
                             }*/
                                ?>
...
var myChart = new Chart(ctx, {
    type: 'bar',
   data:{
   labels: <?php echo json_encode($tolltype); ?>,
        datasets: [{
            label: 'Tolls',
