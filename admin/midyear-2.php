...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <title>Birim Central Municipal Assembly - Area Mid year</title>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-warning">
                        <div class="panel-heading">
                             AREA/ZONES (JANUARY - JUNE)
                        </div>
                        <div class="panel-body">
                          <?php 
                            $sql = "SELECT area, moment, sum((amtc * 100) / (select sum(amtc) from tolltable WHERE moment BETWEEN '".$_SESSION['year']."-01-01 00:00:00' and '".$_SESSION['year']."-06-30 23:59:00')) AS areatotal FROM tolltable LEFT JOIN area ON area.areaid=tolltable.areaid WHERE moment BETWEEN '".$_SESSION['year']."-01-01 00:00:00' and '".$_SESSION['year']."-06-30 23:59:00' GROUP BY area";
                                $result = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($result)) { 
                     
                                $farea[]  = $row['area'];
                                $faramount[] = $row['areatotal']; 
                             }
                             $sql ="SELECT area, moment, sum((paid * 100) / (select sum(paid) from ratetable WHERE moment BETWEEN '".$_SESSION['year']."-01-01 00:00:00' and '".$_SESSION['year']."-06-30 23:59:00')) AS rareatotal FROM ratetable LEFT JOIN area ON area.areaid=ratetable.areaid WHERE moment BETWEEN '".$_SESSION['year']."-01-01 00:00:00' and '".$_SESSION['year']."-06-30 23:59:00' GROUP BY area";
                             $result = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($result)) { 
                                $framount[] = $row['rareatotal']; 
                             }
                                ?>
                            <canvas  id="chartjs_pie1"></canvas>    
                        </div>
                    </div>
                    <!--End Advanced Tables -->
                    <!-- Advanced Tables -->
                    <div class="panel panel-info">
                        <div class="panel-heading">
                             AREA/ZONES (JULY - DECEMBER)
                        </div>
                        <div class="panel-body">
                            <?php 
                            $sql = "SELECT area, moment, sum((amtc * 100) / (select sum(amtc) from tolltable WHERE moment BETWEEN '".$_SESSION['year']."-07-01 00:00:00' and '".$_SESSION['year']."-12-31 23:59:00')) AS sareatotal FROM tolltable LEFT JOIN area ON area.areaid=tolltable.areaid WHERE moment BETWEEN '".$_SESSION['year']."-07-01 00:00:00' and '".$_SESSION['year']."-12-31 23:59:00' GROUP BY area";
                                $result = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($result)) { 
                     
                                $sarea[]  = $row['area']  ;
                                $saramount[] = $row['sareatotal']; 
                             }
                             $sql ="SELECT area, moment, sum((paid * 100) / (select sum(paid) from ratetable WHERE moment BETWEEN '".$_SESSION['year']."-07-01 00:00:00' and '".$_SESSION['year']."-12-31 23:59:00')) AS srareatotal FROM ratetable LEFT JOIN area ON area.areaid=ratetable.areaid WHERE moment BETWEEN '".$_SESSION['year']."-07-01 00:00:00' and '".$_SESSION['year']."-12-31 23:59:00' GROUP BY area";
                             $result = mysqli_query($conn, $sql);
                             $chart_data="";
                             while ($row = mysqli_fetch_array($result)) { 
                                $sramount[] = $row['srareatotal']; 
                             }
                                ?>
