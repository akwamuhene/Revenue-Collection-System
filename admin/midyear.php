...
<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <title>Birim Central Municipal Assembly - collectors Mid year</title>
                ...
                <div class="col-lg-12">
                    ...
                    <div class="panel panel-warning">
                        <div class="panel-heading">
                             REVENUE COLLECTORS TABLE (JAN - JUN)
                        </div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-1q">
                                    ...
                                    <tbody>
                                        <?php 
                                         $sql ="SELECT users.userid, users.fname, users.lname, moment, sum(colltotal) AS uctotal, sum(target) AS tgtotal FROM targets LEFT JOIN users ON users.userid=targets.userid WHERE moment BETWEEN '".$_SESSION['year']."-01-01 00:00:00' and '".$_SESSION['year']."-07-31 23:59:00' GROUP BY users.userid";
                                        $result = mysqli_query($conn, $sql);
                                             while ($row = mysqli_fetch_array($result)) { 
                                             $janjunuctotal+=$row['uctotal'];
                                             $janjuntgtotal+=$row['tgtotal'];
                                             $janjunvariance+=$row['uctotal']-$row['tgtotal'];
                                        ?>
                                        <tr>
                                ...
                                <tr>
                                    <td><b>TOTAL (GH₵)</b></td>
                                    <td><b><?php echo number_format((float)$janjunuctotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$janjuntgtotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$janjunvariance, 2, '.', ''); ?></b></td>
                                </tr>
                                </tfoot>
                                </table>
                            </div>
                    ...
                    <div class="panel panel-info">
                        <div class="panel-heading">
                             REVENUE COLLECTORS TABLE (JUL - DEC)
                        </div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-2q">
                                    ...
                                    <tbody>
                                        <?php 
                                         $sql ="SELECT users.userid, users.fname, users.lname, moment, sum(colltotal) AS uctotal, sum(target) AS tgtotal FROM targets LEFT JOIN users ON users.userid=targets.userid WHERE moment BETWEEN '".$_SESSION['year']."-07-01 00:00:00' and '".$_SESSION['year']."-12-31 23:59:00' GROUP BY users.userid";
                                        $result = mysqli_query($conn, $sql);
                                             while ($row = mysqli_fetch_array($result)) { 
                                             $juldecuctotal+=$row['uctotal'];
                                             $juldectgtotal+=$row['tgtotal'];
                                             $juldecvariance+=$row['uctotal']-$row['tgtotal'];
                                        ?>
                                        <tr>
                                            <td><?php echo $row['fname']; ?> <?php echo $row['lname']; ?></td>
                                            <td><?php echo number_format((float)$row['uctotal'], 2, '.', ''); ?></td>
                                ...
                                <tr>
                                    <td><b>TOTAL (GH₵)</b></td>
                                    <td><b><?php echo number_format((float)$juldecuctotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$juldectgtotal, 2, '.', ''); ?></b></td>
                                    <td><b><?php echo number_format((float)$juldecvariance, 2, '.', ''); ?></b></td>
                                </tr>
                                </tfoot>
                                </table>
                            </div>
